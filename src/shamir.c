...
ssize_t shamir_poly_size(shamir_params_t params){
	if (params_invalid)
		return fail(EINVAL);
	return (params.size * params.threshold) * sizeof(g256_t);
}

ssize_t shamir_key_size(shamir_params_t params){
	if (params_invalid)
		return fail(EINVAL);
	return (params.size + 1) * sizeof(g256_t);
}
...
*/
#define c(params, p, i, j) (*(p + (j * params.threshold) + i))

/*
	The x value of the i-th key
*/
#define k_x(params, k, i) (*_k(params,k,i))

/*
	The j-th y value of the i-th key
*/
#define k_y(params, k, i, j) (*(_k(params,k,i) + j + 1))

/*
	The i-th key
*/
#define k(params, k, i) (k + (i * params.threshold))
...
int shamir_init_poly(shamir_params_t params, shamir_poly_t p, uint8_t *secret){
	...
	ssize_t poly_size = shamir_poly_size;
	if (poly_size == -1) return -1;

#ifdef SHAMIR_URANDOM
...
/* Calculate the key k for the given x value.
	 ...
	 evaluated at x.
 */
int _shamir_get_key(shamir_params_t params, shamir_poly_t *p, gf256 x, shamir_key_t *k){
	if (!p || !key || params_invalid(params) || (x < 1))
		return fail(EINVAL);
	...
	for (unsigned j = 0; j < params.size; j++){
		...
		k_y(params,k,0,j) = (gf256)y;
	}
	return 0;
}

int shamir_get_keys(shamir_params_t params, shamir_poly_t *p, shamir_key_t *k, unsigned nkeys){
	if (!p || !keys || params_invalid(params) || nkeys < 2 || nkeys > MAX_KEYS)
		return fail(EINVAL);
	...
	for (unsigned i = 0; i < nkeys; i++){
		...
		*/
		int ret = _shamir_get_key(params, p, i+1, &k(params,k,i));
		if (ret == -1) return -1;
	}
	return 0;
}
...
int shamir_recover_secret(shamir_params_t params, shamir_key_t *k, uint8_t *secret){
	if (!keys || !secret || params_invalid(params))
		return fail(EINVAL);
	...
	for (unsigned j = 0; j < params.size; j++){
		...
		for (unsigned i = 0; i < params.threshold; i++){
			...
			for (unsigned _i = 0; _i < params.threshold; _i++){
				if (i == _i) continue;
				log_n += log[k_x(params, k, _i)];
				log_d += log[k_x(params, k, i) ^ k_x(params, k, _i)];
			}
